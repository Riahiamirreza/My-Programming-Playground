...
x = (lambda skills: max((((lambda skills: (f := lambda numbers: numbers[0] if len(numbers) == 1 else numbers[0] * f(numbers[1:]))(list(pow(j, i) for i, j in enumerate(sorted(skills), start=2))))(j[0:i]+[j[i]+1]+j[i+1:]), i) for i, j in enumerate([skills]*len(skills))), key=lambda i:i[0])[1])
